...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            // 小平回显数据字典
            selectDictLabel1: function (datas, value) {
                var actions = [];
                $.each(datas, function (index, Lz_dict) {
                    if (Lz_dict.codeNo == ('' + value)) {
                        var listClass = $.common.equals("default", Lz_dict.listClass) || $.common.isEmpty(Lz_dict.listClass) ? "" : "badge badge-" + Lz_dict.listClass;
                        actions.push($.common.sprintf("<span class='%s'>%s</span>", listClass, Lz_dict.codeName));
                        return false;
                    }
                });
                return actions.join('');
        ...
        operate: {
            ...
            // 添加信息，以tab页展现(kk)
            addTab_kk: function (id) {
                $.modal.openTab("添加" + $.table._option.modalName, $.operate.addUrl_kk(id));
            },
            // 添加信息 全屏
            addFull: function (id) {
            ...
            // 添加访问地址（kk）
            addUrl_kk: function (id) {
                var url = $.common.isEmpty(id) ? $.table._option.createUrl1.replace("{id}", "") : $.table._option.createUrl1.replace("{id}", id);
                return url;
            },
            // Cj修改信息
            editCj: function(id) {
                if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = $.table._option.bindUrl.replace("{id}", row[$.table._option.uniqueId]);
                    $.modal.open("修改" + $.table._option.modalName, url);
                } else {
                    $.modal.open("修改" + $.table._option.modalName, $.operate.editUrlCj(id));
                }
            },
            // 修改信息
            edit: function (id) {
			...
			editKan: function(id) {
				if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
					...
					var url = $.table._option.updateUrl.replace("{id}", row[$.table._option.uniqueId]);
					$.modal.openTab("查看" + $.table._option.modalName, url);
				} else {
					$.modal.openTab("查看" + $.table._option.modalName, $.operate.editUrl(id));
				}
			},
            // 修改信息，以tab页展现
            editTabCj: function(id) {
                $.modal.openTab("修改" + $.table._option.modalName, $.operate.editUrlCj(id));
            },
            // >>>>>>>>查看信息
            editKan: function (id) {
                if ($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
            ...
            // Cj修改访问地址
            editUrlCj: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.bindUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
